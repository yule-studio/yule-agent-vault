...
```
SocialProviderType ──→ User.providerType
   (가입 채널)

UserStatus  ──→ User.status            ──(verifyEmail)──→  ACTIVE 로 전이
                                                              │
RefreshTokenStatus ──→ RefreshToken.status                    │
   (ACTIVE / ROTATED / REVOKED)         ←─(login)─────────────┘
                                              │
                                              ▼
                                        login 가능 = (status == ACTIVE)

VerificationTokenStatus ──→ Email/Phone/PasswordReset Token.status
   (4종 공통 enum 또는 각자 별도 — §verification-token-status 참고)

Role ──→ User.role
   (USER / SELLER / ADMIN / MASTER)
   ↓
   JWT claim "role" / Spring Security ROLE_*
```
